...
class CategoryWindow(tk.Toplevel):
    ...
    def on_category_select(self, event):
        ...
        self.entry_nome_cat.insert(0, values[1])
    ...
    def add_category_gui(self):
        ...
        if new_id:
            ...
            self.refresh_main_app_combobox()
        else:
...
class App:
    ...
    def get_form_data(self):
        ...
        if not categoria_nome and not self.categories:
            self.show_feedback("Nenhuma categoria disponível. Cadastre uma categoria primeiro.", "error")
            return None
        elif not categoria_nome and self.categories:
             self.show_feedback("Selecione uma categoria.", "error")
             return None
    ...
    def add_product(self):
        ...
        if data:
            ...
            try:
                new_id = self.db.add_product(nome, tamanho, preco, categoria_id)
                if new_id:
                    self.show_feedback(f"Produto '{nome}' adicionado com sucesso!", "success")
                    self.load_products()
                    self.clear_entries()
                else:
                     self.show_feedback("Erro ao adicionar: Categoria não encontrada.", "error")
            except Exception as e:
    ...
    def update_product(self):
        ...
        if data:
            nome, tamanho, preco, categoria_id = data
            try:
                success = self.db.update_product(self.selected_item_id, nome, tamanho, preco, categoria_id)
                if success:
                    self.show_feedback(f"Produto ID {self.selected_item_id} atualizado!", "success")
                    self.load_products()
                    self.clear_entries()
                else:
                    self.show_feedback("Erro ao atualizar: Produto não encontrado ou Categoria inválida.", "error")
            except Exception as e:
    ...
    def delete_product(self):
        ...
        if messagebox.askyesno("Confirmar Exclusão", f"Tem certeza que deseja excluir o produto ID {self.selected_item_id}?"):
            try:
                success = self.db.delete_product(self.selected_item_id)
                if success:
                    self.show_feedback(f"Produto ID {self.selected_item_id} excluído.", "success")
                    self.load_products()
                    self.clear_entries()
                else:
                    self.show_feedback("Erro ao excluir: Produto não encontrado.", "error")
            except Exception as e:
    ...
    def on_item_select(self, event):
        try:
            ...
            self.entry_tamanho.insert(0, values[2])
            self.entry_preco.insert(0, values[3])
